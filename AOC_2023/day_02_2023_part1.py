def convert_string_to_list(my_string):
    ...
    semi_colons_removed = commas_removed.replace(';', '')
    cleaned_list = semi_colons_removed.split()
    return cleaned_list
...
def get_game_values(list_of_string_words):
    return list_of_string_words[2:]

def game_possible(game_values):
    amount_index = 0
    colour_index = 1

    possible = True

    while colour_index <= len(game_values):
        amount = int(game_values[amount_index])
        colour = game_values[colour_index]

        if (amount > 12) and (colour == 'red'):
            possible = False
        elif (amount > 13) and (colour == 'green'):
            possible = False
        elif (amount > 14) and (colour == 'blue'):
            possible = False
...
def get_game_possibility(my_string):
    list_of_string_words = convert_string_to_list(my_string)
    game_id = get_game_id(list_of_string_words)
    game_values = get_game_values(list_of_string_words)
    result = game_possible(game_values)
...
def calculate_total(input_string):
    list_of_successful_game_ids = []

    list_of_game_strings = input_string.splitlines()
    for my_string in list_of_game_strings:
        # Run each string through the program and get back the id of games that are successful (or 0 if not)
        returned_game_id = get_game_possibility(my_string)

        # Append successful ids to a list
        if returned_game_id != 0:
            successful_game_id = returned_game_id
            list_of_successful_game_ids.append(successful_game_id)

    return(sum(list_of_successful_game_ids))

input_string = '''Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
...
total = calculate_total(input_string)
print(total)