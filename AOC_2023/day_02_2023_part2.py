...
def fewest_number(game_values):
    ...
    while colour_index <= len(game_values):
        current_amount = game_values[amount_index]
        current_colour = game_values[colour_index]

        if current_colour == 'green':
            green_cubes.append(int(current_amount))
        elif current_colour == 'red':
            red_cubes.append(int(current_amount))
        elif current_colour == 'blue':
            blue_cubes.append(int(current_amount))

        amount_index += 2
        colour_index += 2

    fewest_green = max(green_cubes)
    fewest_red = max(red_cubes)
    fewest_blue = max(blue_cubes)

    power = fewest_green * fewest_red * fewest_blue
    return power

def get_fewest_number(my_string):
    list_of_string_words = convert_string_to_list(my_string)
    game_values = get_game_values(list_of_string_words)
    power = fewest_number(game_values)
    return power
...
def calculate_total(input_string):
    ...
    for my_string in list_of_game_strings:
        power = get_fewest_number(my_string)
        total += power
